...
build_phenoFiles = function(row, data, design, out_path){
  ...
  design_matrix$PHE = case_when(design_matrix$PHE == control ~ 1,
                                design_matrix$PHE == case ~ 2,
                                TRUE ~ 0
  )
...
encodings_files = list.files('.', pattern='.json')
encoding_file = encodings_files[str_detect(encodings_files, phenoCol)]

encodings = read_json(encoding_file)
