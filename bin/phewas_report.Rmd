...
The following analysis performs genetic colocalization analysis of two potentially related phenotypes, to ask whether they share common genetic causal variant(s). The hypothesis tested are the following:
...
The plot below will display the Posterior Probability (PP) associated to H4.

Below you will find results regarding colocalisation analysis with the GWAS study selected. If no `coloc` option was set as post_analysis parameter, then you won't find any further results below.

### Colocalization output

```{r}
if (param$coloc_results != 'None') {
table   <- as.data.frame(data.table::fread(params$coloc_results))
colnames(table) <- snakecase::to_snake_case(colnames(table))
DTable(table)
}
if(str_detect(param$coloc_results, '.csv') == FALSE){
...
```{r}
if(str_detect(param$coloc_heatmap, '.png'){

  figure_number  <- "Figure 2: "
  figure_caption <- "Posterior Probability (PP) plot"
  knitr::include_graphics(params$coloc_heatmap, )
  htmltools::tags$figcaption( style = 'caption-side: bottom; text-align: center; font-size: 85%%; color: #71879d',
                              htmltools::em(figure_number),
                              htmltools::em(figure_caption))
}
if(str_detect(param$coloc_heatmap, '.png') == FALSE) {
